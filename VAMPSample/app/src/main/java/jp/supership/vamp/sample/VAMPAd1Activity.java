...
public class VAMPAd1Activity extends BaseActivity {
    ...
    protected void onCreateLayout(Bundle savedInstanceState) {
        ...
        loadButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // 広告の取得を開始します。
                VAMPRewardedAd.load(VAMPAd1Activity.this,
                        VAMP_AD_ID,
                        ...
                        new VAMPRewardedAdLoadAdvancedListener() {
                            ...
                            public void onLoaded(@NonNull String placementId,
                                ...
                                if (success) {
                                    addLog(String.format("onLoaded(%s, %s, true, %s)",
                                            placementId, adNetworkName, message), Color.BLACK);
                                } else {
                                    // 失敗しても、次のアドネットワークがあれば、広告取得を試みます。
                                    // 最終的に全てのアドネットワークの広告在庫が無ければ
                                    // onFailedToLoadのNO_ADSTOCKが通知されます。
                                    addLog(String.format("onLoaded(%s, %s, false, %s)",
                                            placementId, adNetworkName, message), Color.RED);
                                }
                            }
    ...
    protected void onCreate(Bundle bundle) {
        ...
        VAMPActivityEventDispatcher.getInstance()
                .onCreate(bundle, VAMP_AD_ID, rewardedAdListener);
